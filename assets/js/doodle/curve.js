...
export class Curve {
    ...
    genSecondCoordinates( previousCurve, radius ){
        var node_angle = Utils.getRandomFloat(previousCurve.angle + Math.PI,previousCurve.angle + (2*Math.PI));

        this.cp2X = previousCurve.x - radius + (Utils.getRandomInt(-radius/2,radius/2)  *Math.sin(node_angle));
        this.cp2Y = previousCurve.y + (Utils.getRandomInt(-radius/2,radius/2) * Math.cos(node_angle));
    }
    ...
    checkLineIntersection( previousCurve ){
        ...
        denominator = ((this.y - this.cp2Y) * (this.cp1X - previousCurve.x)) - ((this.x - this.cp2X) * (this.cp1Y - previousCurve.y));
        if (denominator == 0) {
            //return result;
        }
        a = previousCurve.y - this.cp2Y;
        b = previousCurve.x - this.cp2X;
        numerator1 = ((this.x - this.cp2X) * a) - ((this.y - this.cp2Y) * b);
        numerator2 = ((this.cp1X - previousCurve.x) * a) - ((this.cp1Y - previousCurve.y) * b);
        ...
        /*
                ...
                x = this.cp2X + (b * (this.x - this.cp2X));
                y = this.cp2X + (b * (this.y - this.cp2Y));
                */
